...
def km_cph_entropy(df_p,df,ls_col,s_subtype,s_plat,s_cell,savedir=f'{codedir}/20220222/Survival_Plots_Both'):
    ...
    if not len(df_st) < 1:
        ...
        if results.summary.p[0] < 0.2:
            ...
            plt.tight_layout()
            fig.savefig(f"{savedir}/KM_{s_title1.replace(' ','_')}_{s_title2.replace(' ','_')}.pdf")
            ...
            with warnings.catch_warnings():
                ...
                try:
                    ...
                    if cph.summary.p[0] < 0.1:
                        ...
                        plt.tight_layout()
                        fig.savefig(f"{savedir}/CPH_{s_title1.replace(' ','_')}_{s_title2.replace(' ','_')}.pdf")
                except:
                    print(f'skipped {s_col}')   
...
def km_cph_all(df_both,df_clin,s_title1,s_title2,s_marker,alpha=0.05,s_time='Survival_time', s_censor='Survival',
    ...
    if pvalue_km < alpha:
        ...
        ax.legend(handles=handles, labels=newlabels,loc='upper right',fontsize='small',frameon=False)#title=f'{s_groups}'
...
def more_plots(adata,df_p,s_subtype,s_type,s_partition,s_cell,n_neighbors,resolution,z_score,linkage,
    ...
    g.savefig(f'{savedir}/clustermap_PlatformandSubtype_{s_type}_{s_partition}_{s_cell}_{s_type}_{n_neighbors}_{resolution}.pdf')
    ...
    g.savefig(f'{savedir}/clustermap_mean_subtypes_{s_type}_{s_partition}_{s_cell}_{s_type}_{n_neighbors}_{resolution}.pdf')
...
def single_km(df_all,s_cell,s_subtype,s_plat,s_col,savedir,alpha=0.05,cutp=0.5,s_time='Survival_time',
    ...
    if len(df) > 0:
        ...
        if results.summary.p[0] < alpha:
            ...
            ax.legend(loc='upper right',title=None,fontsize='small',frameon=False)#{cutp}({i_cut:.2}
            plt.tight_layout()

...
def cropped_ometiff(s_scene,subdir,cropdir,d_crop,d_combos,s_dapi,tu_dim,b_8bit=True,b_resize=False):
    s_sample = s_scene.split('-Scene')[0]
    if os.path.exists(f'{subdir}/{s_sample}'):
    ...
    dd_result = mpimage.overlay_crop(d_combos,d_crop_scene,df_img,s_dapi,tu_dim,b_8bit=b_8bit,b_resize=b_resize)
...
def crop_registered(df_img,bigdir,regdir,d_crop):
    '''
    crop a stack of tiffs to the specified coordinates
    d_crop: crop to scene:(xmin, y_min, xmax, ymax)
    '''
    if df_img.scene.unique()[0].find('-Scene-') > -1:
        s_scene = df_img.scene.unique()[0]
        s_sample = s_scene.split('-Scene')[0]
        df_scene = df_img[df_img.scene==s_scene]
    else:
        s_scene = df_img.slide_scene.unique()[0]
        s_sample = s_scene.split('_')[0]
        df_scene = df_img[df_img.slide_scene==s_scene]
    print(s_scene)
    s_scene_name = re.sub(r'[0-9]+', '', s_scene.split(s_sample)[1])
    print(s_scene_name)
    #old #df_img = mpimage.parse_org()
    ...
    for s_image in df_scene.index:
        ...
        for idx, xy_cropcoor in d_crop.items():
            ...
            preprocess.cmif_mkdir([f'{regdir}/{s_sample}{s_scene_name}{idx:03}'])
            io.imsave(f'{regdir}/{s_sample}{s_scene_name}{idx:03}/{s_image.replace(s_scene,f"{s_sample}{s_scene_name}{idx:03}")}',a_crop,check_contrast=False)
...
def load_crop_labels(d_crop,tu_dim,segdir,cropdir,s_find='Nuclei Segmentation Basins',b_resize=False):
    """
    crop the segmentation basins (cell of nuceli) to same coord as images for veiwing in Napari
    ...
    for s_scene, xy_cropcoor in d_crop.items():
        ...
        if os.path.exists(f'{segdir}/{s_sample}Cellpose_Segmentation/'):
            ...
            #elmars way
            s_sample = s_scene.split('_')[0]
            os.chdir(f'{segdir}/{s_sample}_CellposeSegmentation/')
        for s_file in os.listdir():
            if s_file.find(s_find) > -1: #Nuclei Segmentation Basins.tif #Cell Segmentation Basins.tif
                if s_file.find(s_scene.split(s_sample)[1]) > -1:
                    ...
                    a_crop = a_seg[(xy_cropcoor[1]):(xy_cropcoor[1]+tu_dim[1]),(xy_cropcoor[0]):(xy_cropcoor[0]+tu_dim[0])]
                    if b_resize:
                        #okay for visualization: bug introduced at edges...
                        #a_crop = transform.rescale(a_crop, scale=0.5,preserve_range=True).astype(a_crop.dtype)
                        a_crop = transform.downscale_local_mean(a_crop,(2, 2)).astype(a_crop.dtype)
                    s_coor = f'x{xy_cropcoor[0]}y{xy_cropcoor[1]}.tif'
                    #crop file

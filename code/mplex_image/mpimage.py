...
def parse_org(s_end = "ORG.tif",s_start='R',type='reg'):
    ...
    if sorted(df_img['color'].unique())[0].find('Sub') >-1:
        df_img['color'] = [item[4] for item in lls_name]
    ...
    if [item[3] for item in lls_name][0].find('scene') > -1:
        df_img['scene'] = df_img.scene + '_' + [item[3] for item in lls_name]
    elif [item[3] for item in lls_name][0].find('reg') > -1:
        df_img['scene'] = df_img.scene + '_' + [item[3] for item in lls_name]
...
def overlay_crop(d_combos,d_crop,df_img,s_dapi,tu_dim=(1000,1000),b_8bit=True,b_resize=False): 
    """

...
def load_crops(viewer,s_crop,s_tissue):
    '''
    load ome tiffs and labels that were generated by Jenny's pipeline
    '''
...
def add_slide_name(viewer, title_text, title_font, s_layer='DAPI1'):
    '''
    add text to layer in viewer
    '''
...
def add_points(s_crop,df,s_slide,i_scale = 2):
    '''
    add dots at each cell centroid
    '''
...
def correct_labels(viewer):
    '''
    get rid of pixels between labels that are messed up when downsizing labels
    '''
    cell_boundaries = viewer.layers['cell_boundaries'].data
    new_boundaries = skimage.morphology.binary_erosion(cell_boundaries)
    label_image = viewer.layers['cell_seg'].data
    label_image[new_boundaries] = 0
    return(label_image)

def load_marker(viewer,s_crop,s_tissue,d_marker={}):
    '''
    display markers with equalized display range for making figures
    '''
    ls_color = ['blue','green','yellow','red','cyan','magenta','gray','green','yellow','red','cyan','magenta',
     'gray','gray','gray','gray','gray','gray','gray','gray']
    print(s_crop)
    ls_marker = list(d_marker.keys())
                    ...
                    with tifffile.TiffFile(s_file) as tif:
                        ...
                        for idxs, s_marker in enumerate(ls_marker):
                            if len(set(d_result.keys()).intersection(set([s_marker])).intersection(set(ls_marker_all))) > 0:
                                ...
                                if type(d_marker[s_marker]) == int:
                                    i_max = d_marker[s_marker]
                                    i_min = np.quantile(img,0)
                                else:
                                    i_max = d_marker[s_marker][1]
                                    i_min = d_marker[s_marker][0]
                                if i_max == 0:
                                    i_max = (np.quantile(img,0.9999)+1)*1.5
                                viewer.add_image(img,name=s_marker,rgb=False,visible=True,
                                 blending='additive',colormap=ls_color[idxs],
                                 contrast_limits = (i_min,i_max))
...
def add_points(s_crop,df,s_slide,i_scale = 2):
    '''
    give data frame with slide name in index, and DAPI_X, DAPI_Y columns
    will plot all cells @ spatial locations, scaled down by i_scale
    '''
    x_corr = int(s_crop.split('x')[1].split('y')[0])
    y_corr = int(s_crop.split('x')[1].split('y')[1])
    x_arr = (df.loc[df.index.str.contains(s_slide),['DAPI_X']] - x_corr)/i_scale
...
def add_celltype_points(s_crop,df,s_slide,s_cell,i_scale = 2):
    '''
    give data frame with slide name in index, and DAPI_X, DAPI_Y columns
    plus a boolean column s_cell = celltype
    will plot all cells of that celltype @ spatial locations, scaled down by i_scale
    '''
    x_corr = int(s_crop.split('x')[1].split('y')[0])
    y_corr = int(s_crop.split('x')[1].split('y')[1])
    x_arr = (df.loc[((df.index.str.contains(s_slide)) & (df.loc[:,s_cell])),['DAPI_X']] - x_corr)/i_scale
    y_arr = (df.loc[((df.index.str.contains(s_slide)) & (df.loc[:,s_cell])),['DAPI_Y']] - y_corr)/i_scale
    x_arr['DAPI_Y'] = y_arr
    points = np.array(x_arr.loc[:,['DAPI_Y','DAPI_X']])
    return(points,x_arr)


def points_in_shape(viewer,points,df,s_slide):
    '''
    draw shape in napari viewer, layer must be named "Shapes"
    '''
    verts = viewer.layers['Shapes'].data[0]
    b_poly = measure.points_in_poly(points, verts)
    point_properties = {
        'label': df.loc[df.index.str.contains(s_slide),['cellid']],
        'in_poly' : b_poly
    }
    return(point_properties, b_poly)
...
ls_color = ['green','yellow','red','cyan','magenta','gray'] * 16
def add_cluster(viewer,df_scene,s_col,i_clust,idx,labels):
    ls_index = df_scene[df_scene.loc[:,s_col]==i_clust].index
    ls_index = [int(item.split('_cell')[1]) for item in ls_index]
    labels_cell =  labels.copy()
    b_cell = np.isin(labels, ls_index)
    labels_cell[~b_cell] = 0
    boundaries = skimage.segmentation.find_boundaries(labels_cell).astype('int')
    boundaries[boundaries==1] = 255
    viewer.add_image(boundaries, name=f'cluster{i_clust}',visible=False,blending='additive',colormap=ls_color[idx])

################## thresholding jupyter notbook ##############################
    ...
    for s_scene in ls_scene:
        ...
        for ax_num, s_color in enumerate(ls_color):
            ...
            if len(df_color_pos)>=1:
                ...
                #plot positive cells
                ax[ax_num].scatter(data=df_color_pos, x='DAPI_X',y='DAPI_Y',color='DarkBlue',s=.25)
                      
                ax[ax_num].axis('equal')
                ax[ax_num].set_ylim(ax[ax_num].get_ylim()[::-1])
                ax[ax_num].set_title(f'{s_marker} min={int(s_min)} ({len(df_color_pos)}/{len(df_neg)} cells)')
            else:

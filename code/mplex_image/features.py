...
def parse_org(s_end = "ORG.tif",s_start='R'):
    ...
    try: 
        ...
        df_img['scene'] = [item.split('_scene')[1].split('_')[0] for item in df_img.index]
...
def extract_cellpose_features(s_sample, segdir, subdir, ls_seg_markers, nuc_diam, cell_diam,b_big=False,b_thresh=False): #,
    ...
    fn1 = f'{segdir}/{s_sample}Cellpose_Segmentation'
    fn2 = f'{segdir}/{s_sample}_CellposeSegmentation' #elmar
    ...
    for s_file in os.listdir():
        if s_file.find(f'{".".join(ls_seg_markers)} matchedcell{cell_diam} - Cell Segmentation Basins')>-1:
            ...
            d_match.update({s_file.split('_')[0]:s_file})
            s_type = 'Jenny'
        elif s_file.find(f'{".".join(ls_seg_markers)}_nuc{nuc_diam}_cell{cell_diam}_matched_CellSegmentationBasins.tif')>-1:
            s_scene = s_file.split(f'_{".".join(ls_seg_markers)}')[0]
            ls_scene.append(s_scene)
            d_match.update({s_scene:s_file})
            s_type = 'Elmar'
    print(f'Naming type: {s_type}')
    for s_scene in ls_scene:
        ...
        for s_file in os.listdir():
            if s_type == 'Jenny':
                ...
                if s_file.find(f'{s_scene}_') > -1:
                    if s_file.find("NucleiProjection.png") > -1:
                        s_dapi = s_file
                        print(f'found {s_file}')
        if s_dapi.find(s_scene) == -1:
                print('no dapi found!')
                return
        dapi = io.imread(f'{fn}/{s_dapi}')
        print(f'loading {s_scene} nuclei labels')
        if s_type == 'Jenny':
            labels = io.imread(f'{s_scene} nuclei{nuc_diam} - Nuclei Segmentation Basins.tif')
        elif s_type == 'Elmar':
            labels = io.imread(f'{s_scene}_nuc{nuc_diam}_NucleiSegmentationBasins.tif')
        print(f'loading {d_match[s_scene]}')
        cell_labels = io.imread(f'{d_match[s_scene]}')
        #nuclear features
        ...
        if s_type == 'Jenny':
            ...
            df_feat.index = [f'{s_scene}_cell{item}' for item in df_feat.loc[:,'label_segmented-nuclei']]
        ...
        d_loc = {'nuclei':labels,'cell':cell_labels,'cytoplasm':cyto, 'perinuc5':d_loc_nuc['ring'][1],
         'nucmem':d_loc_nuc['membrane'][0],'cellmem':d_loc_cell['membrane'][0],'exp5':d_loc_nuc['grown'][1],
         'nucadj2':d_loc_nuc['straddle'][0],'celladj2':d_loc_cell['straddle'][0] #take out some regions?
         }
        ...
        for s_index in df_scene.index:
                ...
                for s_loc, a_loc in d_loc.items():
                    ...
                    try:
                        ...
                        df_marker_loc.index = [f'{s_scene}_cell{item}' for item in df_marker_loc.index]
                    df_feat = df_feat.merge(df_marker_loc, left_index=True,right_index=True,how='left',suffixes=('',f'{s_marker}_{s_loc}'))
        if b_big:
            df_feat.to_csv(f'{fn}/features_{s_sample}-{s_scene}.csv')
    ...
    for s_file in sorted(os.listdir()):
        if s_file.find(s_sample) > -1:
            if os.path.isdir(s_file):
                os.chdir(s_file)
                df_img = parse_org()
                img_dapi = find_dapi(df_img, subdir, s_sample)
                ...
                img[border_mask] = 255
                io.imsave(f"{segdir}/TissueEdgeMask{i_pixel}_{s_sample}_scene{df_img.scene.unique()[0]}.png", img)
                os.chdir('..')
                
def area_mask(s_sample,segdir,subdir,s_round, dapi_thresh=350,i_fill=50000,i_close=20):
    '''
    find edge of the tissue. first, find tissue by threshodling DAPI R1 (pixels above dapi_thresh)
    then, mask all pixels within i_pixel distance of tissue border
    return/save binary mask
    ...
    for s_file in sorted(os.listdir()):
        if s_file.find(s_sample) > -1:
            #if os.path.isdir(s_file): #spacial for JP-TMA all in one folder!
            os.chdir(s_file)
            df_img = parse_org()
            for s_scene in sorted(set(df_img.scene)):   
                df_scene = df_img[df_img.scene==s_scene] 
                img_dapi = find_dapi(df_scene, subdir, s_sample,s_round)
                mask = img_dapi > dapi_thresh 
                mask_small = morphology.remove_small_objects(mask, min_size=100)
                mask_closed = morphology.binary_closing(mask_small, morphology.octagon(i_close,i_close//2))
                mask_filled = morphology.remove_small_holes(mask_closed, i_fill)
                #border_mask, __, __,distances = mask_border(mask_filled,type='inner',pixel_distance=i_pixel)
                img = np.zeros(mask_filled.shape,dtype='uint8')
                img[mask_filled] = 255
                io.imsave(f"{segdir}/TissueMask_{s_sample}_scene{s_scene}.png", img)
                #io.imsave(f"{segdir}/TissueMask_{s_sample}_scene{df_img.scene.unique()[0]}.png", img)
            os.chdir('..')
 
def find_dapi(df_img, subdir, s_sample,s_round='R1'):
    for s_scene in sorted(set(df_img.scene)):
        print(f'Calculating mask for Scene {s_scene}')
        s_index = df_img[(df_img.scene == s_scene) & (df_img.rounds == s_round) & (df_img.color =='c1')].index[0]
        ...
        elif os.path.exists(f'{subdir}/{df_img.loc[s_index,"slide_scene"]}/{s_index}'): #elmar
            img_dapi = io.imread(f'{subdir}/{df_img.loc[s_index,"slide_scene"]}/{s_index}')
            try:
                s_scene = s_scene.split('scene')[1]
            except:
                print('')
        else:
            print('no DAPI found')
...
def edge_bbox(s_sample,segdir,i_pixel=154):
    '''
    load a binary mask of tissue, find largest tissue piece
    return data frame of bbox (min_row, min_col, max_row, max_col)
    '''
    df_sample = pd.DataFrame()
    #load masks
    for s_file in sorted(os.listdir(segdir)):
        if s_file.find(f'TissueEdgeMask{i_pixel}_{s_sample}_scene') > -1:
            s_scene = f'scene{s_file.split("scene")[1].split(".png")[0]}'
            print(f'Calculating bbox for {s_file}')
            mask = io.imread(f"{segdir}/{s_file}")
            label_image = measure.label(mask)
            df_props = pd.DataFrame(measure.regionprops_table(label_image, properties=('label', 'bbox','area')))
            se_large = df_props.sort_values('area').iloc[-1]
            minr, minc, maxr, maxc = se_large.loc['bbox-0'],se_large.loc['bbox-1'],se_large.loc['bbox-2'],se_large.loc['bbox-3']
            fig,ax=plt.subplots()
            ax.imshow(mask)
            bx = (minc, maxc, maxc, minc, minc)
            by = (minr, minr, maxr, maxr, minr)
            ax.plot(bx, by, '-b', linewidth=2.5)
            #old fig.savefig(f'{segdir}/TissueEdgeMask{i_pixel}_{s_sample}-Scene-{re.sub("[a-zA-Z]+", "", s_scene)}_bbox.png')
            fig.savefig(f'{segdir}/TissueEdgeMask{i_pixel}_{s_sample}-Scene-{s_scene.replace("scene","")}_bbox.png')
            se_large.name = f'{s_sample}_{s_scene}'
            df_sample = df_sample.append(se_large)
            #break
    return(df_sample)
...
def edge_cells(s_sample,segdir,nuc_diam,i_pixel=154):
    ...
    for s_scene in ls_scene:
        ...
        try: #load mask
            ...
            mask_gone = True
        fn = f'{segdir}/{s_sample}Cellpose_Segmentation/{s_sample}-Scene-{s_scene} nuclei{nuc_diam} - Nuclei Segmentation Basins.tif'
        fn2 = f'{segdir}/{s_sample}_CellposeSegmentation/{s_sample}_scene{s_scene}_nuc{nuc_diam}_NucleiSegmentationBasins.tif'
        label_gone = False
        if os.path.exists(fn): #load lables
            labels = io.imread(fn)
        elif os.path.exists(fn2): #elmar's labels
            labels = io.imread(fn2)
        else:
            label_gone = True
        if label_gone or mask_gone:
            print(f'Files not found for {s_scene}')
        else:
            #all files found
            mask_gray = mask == 255
